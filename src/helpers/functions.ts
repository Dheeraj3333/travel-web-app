...
export function querySerialize<T>(
  queryObj?: Record<string, string | number | boolean | (string | number | boolean)[]> | T
): string {
  if (!queryObj) {
    return "";
  }
...
export const serializeParams = (obj: Record<string, any>, prefix = ''): URLSearchParams => {
...
export function setCookie(name: string, value: string, minutesToExpire: number, path = '/', domain = '', secure = false) {
  const date = new Date();
  ...
  document.cookie = `${name}=${encodeURIComponent(value)}; ${expires}${cookiePath}${cookieDomain}${cookieSecure}; SameSite=Lax`;
}
